...
\emph{Types} gained one more base since: $\mathsf{ord}$, the ordinal
notations below $\epsz$ as an \emph{inductive} type rather than a code. In
the first-order development a notation is a natural number built by iterated
pairing, and every proof about it carries decode and normal-form side
conditions; here it is a tree, comparison is pattern matching, and
well-foundedness is \emph{inherited} from the certified coded order by
pulling back along the evident encoding --- so the coding survives only
inside one well-foundedness proof and appears in no computation at all.
Section~\ref{sec:haomega} closes with what that buys.
...
too. The ordinals are out (above); the hydra trees are not, and porting them
