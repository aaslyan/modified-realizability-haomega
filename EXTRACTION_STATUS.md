...
This report provides the audited status of program extraction in `HAomega` across all 368 `#guard`/`#eval` verification points, strictly categorizing each into **EXTRACTED**, **OBJECT-RUN**, and **PLAIN**, and detailing the mathematical specification content, tautological status, and soundness guarantees.

---

## A. Headline Table

Across `HAomega/*.lean`, verified proof extraction (**EXTRACTED**) accounts for **96** guards:

| Tier | Count | Description |
|---|---|---|
| **EXTRACTED** | **96** | Programs extracted via `extractClosed (d : Deriv)` from proof derivations |
...
| **Total** | **368** | All kernel verification points across the entire library |

### Raw `classify.py` Output

```
TOTALS: {'EXTRACT': 96, 'Tm.eval': 30, 'PLAIN': 242} sum: 368
...
SquareRoot.lean                  {'EXTRACT': 26, 'Tm.eval': 0, 'PLAIN': 1}
...
1. **GENUINE (Constructive Content in Formula):**
   The theorem statement asserts a substantive non-tautological property (e.g. root bracketing, cube roots, polynomial inverses, modulus of continuity), and the proof constructs the algorithm to fulfill it without being handed the answer.
...
|---|---|---|---|---|---|---|
| `sqrtApproxX` | `SquareRoot.lean:91` | `sqrtApproxD` | $\exists k < K.\, \mathrm{sqCol}(k) \neq \mathrm{sqCol}(k+1)$ | **GENUINE** (Discrete IVT square root crossing) | `[propext, Quot.sound]` | $\sqrt{2} \approx \frac{22}{16}, \frac{362}{256}; \sqrt{9}=3; \sqrt{1/4}=1/2$ |
| `cubeApproxX` | `SquareRoot.lean:159` | `cubeApproxD` | $\exists k < K.\, \mathrm{cubeCol}(k) \neq \mathrm{cubeCol}(k+1)$ | **GENUINE** (Target C1: Cube root crossing) | `[propext, Quot.sound]` | $\sqrt[3]{2} \approx \frac{20}{16}, \frac{322}{256}; \sqrt[3]{8}=2; \sqrt[3]{27}=3; \sqrt[3]{1/8}=1/2$ |
| `fnCrossingX` | `SquareRoot.lean:225` | `fnCrossingD` | $\exists k < K.\, \mathrm{fnCol}(F, y, k) \neq \mathrm{fnCol}(F, y, k+1)$ | **GENUINE** (Targets C2 & C4: General root & inverse locator) | `[propext, Quot.sound]` | $x^4=2 \implies x \approx \frac{19}{16}; 2x+1=5 \implies x=2; x^3+x=10 \implies x=2; x^5=32 \implies x=2$ |
...
| `picardAffineExtracted` | `ODEDemo.lean:148` | `picardAffineDeriv` | $\forall n.\, \exists y.\, y = \mathrm{recNat}\ 1\ (\lambda \_ y.\, 1 + y/2)\ n$ | **TAUTOLOGICAL** ($\forall n. \exists y. y = y_n$) | `[propext, Quot.sound]` | $T^0(1) \dots T^4(1) = 1, \frac{3}{2}, \frac{7}{4}, \frac{15}{8}, \frac{31}{16}$ |
...
1. **Non-Tautological Constructive Analysis Highlights:**
   * **Square Root Bracketing:** [`SquareRoot.lean`](file:///Users/araaslyan/modified-realizability-haomega/HAomega/SquareRoot.lean) proves discrete IVT sign-crossing for squaring and extracts binary root search for $\sqrt{2}, \sqrt{9}, \sqrt{1/4}$.
   * **Cube Root Extraction (Target C1):** [`SquareRoot.lean`](file:///Users/araaslyan/modified-realizability-haomega/HAomega/SquareRoot.lean) proves discrete IVT sign-crossing for cubing and extracts $\sqrt[3]{2}, \sqrt[3]{8}, \sqrt[3]{27}, \sqrt[3]{1/8}$.
   * **General Root & Inverse Function Extraction (Targets C2 & C4):** [`SquareRoot.lean`](file:///Users/araaslyan/modified-realizability-haomega/HAomega/SquareRoot.lean) proves the general IVT crossing theorem for arbitrary computable functions $F : \mathbb{Q} \to \mathbb{Q}$ and target $y$, extracting certified solutions for 4th roots ($x^4 = 2$), affine inverses ($2x+1 = 5$), cubic inverses ($x^3+x=10$), and 5th roots ($x^5=32$).
...
2. **Computational Verification:**
   * All 96 extracted programs evaluate inside the Lean 4 kernel with 0 unproved axioms (`[propext, Quot.sound]` only) and compile to standalone Haskell via `EmitHaskell`.
3. **Build Status:**
