...
For each of the fifteen case studies this module renders the realizer three
...
def R15 := extractClosed (uniContD (Γ := []) (Δ := Ctx.nil))
...
def showAll : String :=
  ...
  section' "15. Uniform continuity — the realizer is the modulus"
    "∀f^(ℚ→ℚ) ∀j. Lipschitz(f,j) → ∀n ∃M ∀x∀y. close(M,x,y) → close(n, f x, f y)"
    "(Q → Q) → (N → ((Q → (Q → (N → (1 → 1)))) → (N → (N × (Q → (Q → (1 → 1)))))))" R15
...
#guard [hsSupported R1, hsSupported R2, hsSupported R3, hsSupported R4,
        ...
        hsSupported R13, hsSupported R14, hsSupported R15]
