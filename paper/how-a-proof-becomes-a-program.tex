...
\title{\bfseries How a Proof Becomes a Program\\[6pt]
  \normalfont\large\itshape A tour of two formal systems in which theorems,
  once proved, hand you a working program\\ --- and of how the proof, its
  logical strength, and its choice of representation\\ each leave a mark on
  the program you get.}
\author{Ara Aslyan}
\date{}
...
\begin{abstract}
\noindent
We present a mechanized framework for modified realizability over finite-type
intuitionistic arithmetic, formalized in Lean~4, in which the extracted
realizer is a term of the object language rather than an opaque function. The
framework proves extraction sound and extracted realizers continuous, and
extends the object theory by a \emph{matched} proof/program pair --- transfinite
induction up to $\epsz$ together with a recursor along the induced ordinal
order --- so that theorems beyond arithmetical induction still yield running
programs; case-study symbols enter as primitives evaluated by separately
verified value layers.

Thirteen theorems are derived and extracted, among them Goodstein's theorem and
the Kirby--Paris hydra theorem in a strategy-quantified form the one-sorted
setting cannot state. The system makes proof structure, logical strength, and
representation observable in the extracted programs: two proofs of one theorem
extract to programs that return different witnesses; replacing a numerical
encoding of hydra trees by a structural type turns a battle whose coded extract
exhausts the evaluator into one that computes the published length, while an
analogous change for ordinals reduces representation size without any measured
change in running time; and type-$2$ realizers are continuous, with a
discontinuous functional exhibited that no derivation extracts to. A
translation into a functional target language is proved correct against a
formal semantics of that target for the non-transfinite fragment. Every
extracted program that runs is machine-checked to depend on no choice
principle.
\end{abstract}
...
apply that lens \emph{mechanically}, turning a verified proof in a formal
proof language built for extraction into runnable code with no human
cleverness in between.\footnote{The development this tour
...
That is Part~I, and it answers the question it sets. But it answers it about a
system deliberately built to be as poor as possible: one sort of object, a
short list of symbols, and everything else --- ordinals, trees, sequences of
moves --- \emph{coded} into a natural number. Poverty keeps the audit short,
which is why we start there. It also has a price, and until you have a second
system to compare against, that price is invisible. Part~II builds the machine
again over a language with types, runs the same theorems through it, and
measures what changed. The comparison turns out to be the more interesting
half: it makes proof structure, logical strength, and representation into
things one can vary one at a time and watch the consequences in the extracted
program.

%======================================================================
\part{A minimal system, end to end}
...
Everything in Part~I happens inside a fixed, small formal system --- we
...
\subsection*{The fragment, formally}

Before defining realizers, we should say exactly what counts as a proof in
this paper. The object theory is not arbitrary Lean, not Mathlib, and not full
higher-type arithmetic. It is a first-order intuitionistic theory of natural
number terms, extended by a small number of named recursive functions and one
transfinite-induction rule. In the Lean development this theory is the
inductive family
\[
  \mathsf{Deriv}(\Gamma,\varphi),
\]
whose inhabitants are finite proof trees from context $\Gamma$ to conclusion
$\varphi$. The extraction theorem quantifies over those trees.

The terms are natural-number-valued:
\[
\begin{array}{rcl}
t,u &::=& x \mid 0 \mid S(t) \mid t+u \mid t\cdot u
      \mid \mathsf{pred}(t) \mid t^u                                          \\
    && {}\mid \mathsf{bump}(t,u) \mid \mathsf{good}(t,u)
      \mid \mathsf{prec}(t,u) \mid \mathsf{ord}(t,u)                          \\
    && {}\mid \mathsf{hcut}(t,u) \mid \mathsf{hydra}(t,u)
      \mid \mathsf{hord}(t)                                                    \\
    && {}\mid \mathsf{hcons}(t,u) \mid \mathsf{happ}(t,u)
      \mid \mathsf{mvcount}(t)                                                 \\
    && {}\mid \mathsf{solves}(t,u,v,w,z) \mid \mathsf{xor}(t,u)
      \mid \mathsf{pas}(t,u)                                                   \\
    && {}\mid \mathsf{look}(t,u) \mid \mathsf{fib}(t).
\end{array}
\]
Variables range only over $\Nat$. The hydras, ordinals, Hanoi move lists,
Pascal entries, and finite colourings that appear later are all represented
by natural-number codes. The fragment has no object-language variable of type
$\Nat\to\Nat$, no lambda abstraction, and no quantifier over functions.

Formulas are generated by
\[
  \varphi,\psi ::= \bot \mid t=u \mid
  \varphi\land\psi \mid \varphi\lor\psi \mid
  \varphi\to\psi \mid \forall x.\,\varphi \mid \exists x.\,\varphi .
\]
Negation is abbreviation: $\neg\varphi$ means $\varphi\to\bot$.

The logical rules are ordinary intuitionistic natural deduction: assumption
and weakening; introduction and elimination for $\land,\lor,\to,\bot$;
introduction and elimination for $\forall$ and $\exists$, with the usual
freshness and capture-avoidance side conditions. The arithmetic rule is
ordinary induction over natural numbers:
\[
  \frac{\Gamma\vdash \varphi(0)
        \qquad
        \Gamma\vdash \forall x.\bigl(\varphi(x)\to\varphi(Sx)\bigr)}
       {\Gamma\vdash \forall x.\varphi(x)}.
\]
This is the rule used for the Fibonacci iterator, Pascal parity, Euclid,
Sperner, and the ordinary recursive parts of Hanoi.

The nonlogical part is a fixed list of schema families. There are equality
schemas (reflexivity, symmetry, transitivity, congruence for every function
symbol), decidable equality of terms, $S(t)\neq 0$, and injectivity of $S$.
There are defining equations for $+$, $\cdot$, predecessor, exponentiation,
the Goodstein sequence, the Hydra battle, Hanoi solutions, Pascal parity,
finite colour lookup, and Fibonacci. Some functions whose recursion follows
the structure of coded objects enter by numeral graphs rather than by a
single first-order recursion equation: hereditary base change $\mathsf{bump}$,
single first-order recursion equation: hereditary base change $\mathsf{bump}$,
ordinal comparison $\mathsf{prec}$, Hydra cutting $\mathsf{hcut}$, and
exclusive-or $\mathsf{xor}$.
These graph schemas are still part of the formal theory: they are explicit
constructors of $\mathsf{Deriv}$, not Lean theorems silently used behind the
scenes.

Finally, the fragment has one genuinely stronger induction principle:
transfinite induction below $\epsz$, expressed over natural-number codes for
ordinal notations:
\[
  \frac{
    \Gamma\vdash
    \forall x.\Bigl(
      \bigl(\forall y.\,(\mathsf{prec}(y,x)=1\to\varphi(y))\bigr)\to\varphi(x)
    \Bigr)}
  {\Gamma\vdash \forall x.\varphi(x)}.
\]
This rule is the extra strength behind Goodstein and the Hydra termination
argument. It is also the main reason the fragment is interesting: the same
extraction and continuity theorem covers ordinary induction and this
$\epsz$-recursion rule uniformly.

This puts the fragment in a slightly unusual but deliberate position. As a
\emph{language of statements}, it is weaker than Peano arithmetic or Heyting
arithmetic: its variables range only over natural numbers, its function
symbols are fixed in advance, and it does not contain a general theorem
saying that every primitive-recursive definition may be added. As a
\emph{logic}, it is intuitionistic rather than classical, so it has no
general excluded middle. As a \emph{higher-type theory}, it is far weaker
than HA$^\omega$, because the object language cannot quantify over
finite-type functionals at all. The higher types appear on the
\emph{realizer} side, not as objects quantified over by the theorem language.

At the same time, the fragment is not meant to be a weak subsystem chosen at
random. It is a purpose-built extraction laboratory. It keeps the object
language first-order and mechanically inspectable, but equips it with exactly
the principles needed by the case studies: ordinary induction for
primitive-recursive mathematics, explicit recursion equations for the coded
objects under study, and one high-strength rule, transfinite induction below
$\epsz$, for ordinal-descent arguments such as Goodstein. In that sense the
comparison with PA or HA is not a single linear strength comparison. The
fragment has less general arithmetic infrastructure than either theory, but
it has a targeted transfinite rule that lets it prove examples ordinary PA
cannot prove in its own language.

The paper's formal claim is therefore:
\begin{keystmt}
For every derivation $D:\mathsf{Deriv}(\Gamma,\varphi)$ in this fragment, the
function $\extract(D)$ computes a modified realizer of $\varphi$ from
realizers of the assumptions in $\Gamma$. In particular, for every closed
derivation $D:\mathsf{Deriv}(\emptyset,\varphi)$, $\extract(D)$ is a certified
program realizing $\varphi$; at type level $2$ its extensional collapse is a
continuous functional.
\end{keystmt}
Everything later in the paper is an instance of this statement. Fibonacci
uses ordinary induction; Hanoi uses ordinary induction plus its solution
schemas; Goodstein uses the same extraction function but feeds it a proof
whose recursion principle is transfinite induction below $\epsz$.
...
proof in the fragment, is continuous.
\end{keystmt}
...
proofs we feed it. In the implementation, those proofs are explicit
\code{Deriv} trees of the fragment; ordinary Lean theorems become input to
this pipeline only after they have been represented and proved in that object
language.
...
Appendix~\ref{app:xray} shows Program B as a tree.}
\label{fig:search}
\end{figure}
...
program in the fragment --- the Goodstein stopping-step function, the
...
%======================================================================
\section{Act VII --- What the small system shows}
...
Appendix~\ref{app:xray} prints this program in full, verbatim from the proof.}
\label{fig:curry-howard}
\end{figure}

The displayed tree is useful because it shows the \emph{shape} of the program.
The repository also exposes ordinary Lean functions obtained by evaluating the
realizer at a certified ambient level and reading the relevant witness or tag.
For the small examples this is short enough to print directly:

\begin{center}
\fcolorbox{rulec}{boxbg}{\begin{minipage}{0.92\linewidth}
\ttfamily\small
def fibonacci (n : Nat) : Nat :=\\
\quad fstPT (fstPT (app1 fibPairedTheorem 4 n)) (defaultPT 4)\\[3pt]
def pasTag (n k : Nat) : Nat :=\\
\quad tag2 pasTotal 6 n k\\[3pt]
def goodsteinStopTime (m : Nat) : Nat :=\\
\quad witness1 goodsteinTheorem 11 m
\end{minipage}}
\end{center}

\noindent
These are not separate algorithms written after the proofs. Each is a reader
for the realizer produced by the same generic extractor. The accompanying
continuity theorem has the uniform form
\[
  \code{extract\_continuous}\ D\ \rho :
  \mathsf{Continuous2}\bigl(\extract(D,\rho,[],1)\bigr),
\]
and the collapse wrapper packages the same type-$2$ realizer as an element of
$\CtQ 2$. Appendix~\ref{app:catalogue} lists the exported functions for the
case studies; Appendix~\ref{app:xray} prints the larger Goodstein program tree
that those one-line readers are reading from.
...
the fragment: everything that runs rests on two inert axioms and nothing
...
So the small system delivers what Act~I promised. But notice what the last two
acts have been quietly reporting. The level bookkeeping was extorted rather
than designed; the pairing had to be hand-built or the audit would have failed;
the Hydra's general theorem could not be \emph{stated} inside the fragment at
all. Three complaints, three coats on one body: the object language has a
single sort, so every structured thing --- a strategy, a colouring, an ordinal,
a tree --- has to be coded into a number before the fragment can mention it.

That the machine still works under those conditions is the achievement of
Part~I. What the coding \emph{cost} is a question Part~I has no way to answer,
because it has nothing to compare against. Part~II builds the comparison.

%======================================================================
\part{The same proof theory, native types}

\input{part2-haomega}

\appendix
\section{Appendix A --- Looking inside: the extracted program as a tree}
\label{app:xray}

Throughout the tour we have said that the extracted realizer \emph{is} a
program, and Act~VII drew it as one (Figure~\ref{fig:curry-howard}). This
appendix closes the loop: it shows the actual program, printed from the actual
proof, by a command you can run.

There is an obstacle to seeing it that is worth stating, because overcoming it is
the point. You cannot simply \emph{evaluate} the extracted realizer and look at
the result. For Goodstein the realizer is built by the transfinite recursion
$\mathrm{tiRecC}$, and evaluating it forces that recursion --- which is exactly
the astronomically long computation Table~\ref{tab:stop} could not finish. And
even on the smallest example, full evaluation \emph{over}-reduces: the realizer of
$\exists s.\ \good(3,s){=}0$ computes to the single number $30$ (it is
$\mathsf{Nat.pair}\,5\,0$, the witness paired with a contentless payload), so the
$5$ the proof supplied is hidden inside an opaque code.

The command \code{\#realizer} avoids both problems by reading the \emph{proof}
structurally rather than running the program: it walks the finite derivation,
names the combinator at each rule, and collapses every proof-irrelevant
sub-derivation (an equation, $\bot$) to a single ``$\cdot$'' leaf. It therefore
terminates instantly on proofs whose programs cannot be evaluated, and it keeps
the witness visible. On the smallest realizer it prints the pair the prose has
described all along:

\begin{center}
\fcolorbox{rulec}{boxbg}{\begin{minipage}{0.82\linewidth}
\ttfamily\small
\begin{tabular}{@{}l@{}}
\input{xray-small.tex}
\end{tabular}
\end{minipage}}
\end{center}

\noindent
The witness $5$ sits at the top, exactly where $\exists$-introduction is pairing;
the body, being an equation, carries no runtime content and is one leaf. That is
Reading~1 of Figure~\ref{fig:curry-howard}, now as literal output.

The same command, on Goodstein's \emph{theorem}, prints the whole extracted
stopping-step program --- the one Figure~\ref{fig:curry-howard} distilled and
Figure~\ref{fig:search} contrasted with a search. It is a $29$-node tree, and it
prints in well under a second, though the program it describes could never be run
to completion at $m=4$:

\begin{center}
\fcolorbox{rulec}{boxbg}{%
  \resizebox{0.96\linewidth}{!}{%
    \ttfamily\footnotesize
    \begin{tabular}{@{}l@{}}
    \input{xray-goodstein.tex}
    \end{tabular}}}
\end{center}

\noindent
Read it top to bottom and it is the program of Figure~\ref{fig:curry-howard}: bind
the input ($\mathsf{allI}$), name the initial ordinal, recurse along $\prec$
($\mathsf{tiEps0}/\mathrm{tiRecC}$), and at each state case-split
($\mathsf{orE}$) on a decided equality --- return the current step
($\mathsf{exI}$, the witness) if the value is $0$, otherwise recurse on the next
state, invoking the induction hypothesis ($\mathsf{ax}$, the recursive call) and
discharging the ordinal descent as a contentless leaf ($\code{prec}(\dots){=}1$).
This is the recursion whose termination Figure~\ref{fig:ordinals} certified and
whose guarantee Figure~\ref{fig:search} set against the naive loop. The Hydra's
program is the same $29$-node shape, for the reason Figure~\ref{fig:hydra-desc}
gives.

\paragraph{Reproducing this.} Both boxes are verbatim output. In the repository,
...
\noindent
prints the two trees above; the companion \code{\#realizerCH} annotates each node
with its logic rule and proposition, which is the three-column correspondence of
Figure~\ref{fig:curry-howard} generated mechanically. The command reads every
derivation rule (it is total by construction, with no wildcard case), so it works
unchanged on every theorem in the development.

For readers who want a less tree-like view, the repository also provides the
generic command \code{\#program}. It runs the same derivation-guided traversal
but renders the extracted realizer as pseudocode: \code{ind} becomes
\code{natRec}, \code{tiEps0} becomes \code{wellFoundedRecOnPrec},
\code{orE} becomes \code{case split}, \code{exI} becomes
\code{return witness}, and equation proofs become \code{erase(...)}. This is a
decompiled display of the extracted program, not a second certified artifact;
the certified object is still \code{extract D}, with correctness from
soundness and continuity from \code{extract\_continuous}.

\section{Appendix B --- Catalogue of exported extracted programs}
\label{app:catalogue}

The previous appendix prints the realizer tree. This appendix records the
ordinary functions exported by the case studies. Each row has the same form:
a theorem in the fragment, a reader that applies the extracted realizer and
pulls out a witness or tag, a correctness theorem obtained from soundness, and
the uniform continuity/\CtQ wrapper.

\begin{center}
\renewcommand{\arraystretch}{1.22}
\scriptsize
\begin{tabular}{@{}p{0.13\linewidth}p{0.28\linewidth}p{0.19\linewidth}p{0.24\linewidth}@{}}
\toprule
\textbf{Case} & \textbf{Extracted reader} & \textbf{Correctness theorem} & \textbf{Continuous program} \\
\midrule
Fibonacci &
\code{fibonacci : Nat -> Nat}\newline
\code{fibNext : Nat -> Nat} &
\code{fibonacci\_spec}\newline
\code{fibNext\_spec} &
\code{fib\_extract\_continuous}\newline
\code{fibRealizesCtQ} \\
\addlinespace[2pt]
Pascal mod $2$ &
\code{pasTag : Nat -> Nat -> Nat}\newline
\code{pasDecide : Nat -> Nat -> Nat} &
\code{pasDecide\_eq} &
\code{pas\_extract\_continuous}\newline
\code{pasRealizesCtQ} \\
\addlinespace[2pt]
Tower of Hanoi &
\code{hanoiSolution : Nat -> Nat -> Nat -> Nat -> Nat}\newline
\code{hanoiMoves : Nat -> List (Nat * Nat)} &
move-count guards and the realized Hanoi specification &
\code{hanoi\_extract\_continuous}\newline
\code{hanoiRealizesCtQ} \\
\addlinespace[2pt]
Euclid / gcd &
\code{gcdWitness : Nat -> Nat -> Nat} &
\code{gcdWitness\_dvd} &
\code{gcd\_extract\_continuous'}\newline
\code{gcdRealizesCtQ} \\
\addlinespace[2pt]
Sperner &
\code{spernerWitness : Nat -> Nat -> Nat} &
\code{spernerWitness\_spec} &
\code{sperner\_extract\_continuous'}\newline
\code{spernerRealizesCtQ} \\
\addlinespace[2pt]
Goodstein &
\code{goodsteinStopTime : Nat -> Nat} &
\code{goodsteinStopTime\_spec} &
\code{goodstein\_extract\_continuous}\newline
\code{goodsteinRealizesCtQ} \\
\addlinespace[2pt]
Hydra &
\code{hydraBattleLength : Nat -> Nat} &
\code{hydraBattleLength\_spec} &
\code{hydra\_extract\_continuous}\newline
\code{hydraRealizesCtQ} \\
\bottomrule
\end{tabular}
\end{center}
...
\[
  \forall D:\mathsf{Deriv}(\emptyset,\varphi).\quad
  \mathsf{Continuous2}\bigl(\extract(D,\rho,[],1)\bigr).
\]
The first column differs from example to example because the theorem shapes
differ. A theorem of shape $\forall x.\exists y.\varphi$ yields a witness
reader like \code{goodsteinStopTime}; a theorem of shape
$\forall x.\forall y.(\varphi\lor\psi)$ yields a tag reader like
\code{pasTag}; a theorem whose conclusion packages two existentials, such as
Fibonacci's strengthened invariant, yields two readers, \code{fibonacci} and
\code{fibNext}. What changes is only how we read the realizer. The extractor,
soundness theorem, continuity proof, and \CtQ collapse are the same.
...
%======================================================================
\section{Appendix C --- The realizer as a term}
\label{app:term}

Appendix~A had to work around an obstacle: in the fragment you cannot see the
extracted program by evaluating it. Evaluation either over-reduces (the witness
disappears into an opaque code) or does not finish (the transfinite recursion
is the astronomical computation itself). So \code{\#realizer} reads the
\emph{proof} instead, walking the derivation to print a tree.

In Part~II that obstacle is gone, and its absence is the structural point of
Act~VIII. The realizer is a term of the object language, so printing it is
printing --- no derivation walk, no separate viewer, and the same object that
prints is the one that runs and the one the certified translation of Act~XII
compiles. Here is a complete extracted program, verbatim: the Pascal
mod-$2$ decision procedure, whose output is the Sierpi\'nski gasket drawn in
Act~IV. Contentless components are elided (the collapsed view); nothing else
is abridged.

\begin{center}
\fcolorbox{rulec}{boxbg}{\begin{minipage}{0.93\linewidth}
\ttfamily\scriptsize\raggedright
($\lambda$x0. rec[($\lambda$x1. rec[0 | ($\lambda$x2. ($\lambda$x3. S 0)) | fst (($\lambda$x2. rec[0 | ($\lambda$x3. ($\lambda$x4. S 0)) | x2]) x1)]) | ($\lambda$x1. ($\lambda$x2. ($\lambda$x3. rec[0 | ($\lambda$x4. ($\lambda$x5. rec[0 | ($\lambda$x6. ($\lambda$x7. S 0)) | fst (($\lambda$x6. rec[S 0 | ($\lambda$x7. ($\lambda$x8. rec[S 0 | ($\lambda$x9. ($\lambda$x10. 0)) | fst x8])) | x6]) (((($\lambda$x6. rec[($\lambda$x7. rec[S 0 | ($\lambda$x8. ($\lambda$x9. 0)) | x7]) | ($\lambda$x7. ($\lambda$x8. ($\lambda$x9. rec[S 0 | ($\lambda$x10. ($\lambda$x11. ((($\lambda$x12. ($\lambda$x13. (($\lambda$x14. rec[0 | ($\lambda$x15. ($\lambda$x16. (($\lambda$x17. rec[S 0 | ($\lambda$x18. ($\lambda$x19. 0)) | x17]) x16))) | x14]) (x12 + x13)))) (x8 x10)) (x8 S x10)))) | x9]))) | x6]) x1) x4) + ((($\lambda$x6. rec[($\lambda$x7. rec[S 0 | ($\lambda$x8. ($\lambda$x9. 0)) | x7]) | ($\lambda$x7. ($\lambda$x8. ($\lambda$x9. rec[S 0 | ($\lambda$x10. ($\lambda$x11. ((($\lambda$x12. ($\lambda$x13. (($\lambda$x14. rec[0 | ($\lambda$x15. ($\lambda$x16. (($\lambda$x17. rec[S 0 | ($\lambda$x18. ($\lambda$x19. 0)) | x17]) x16))) | x14]) (x12 + x13)))) (x8 x10)) (x8 S x10)))) | x9]))) | x6]) x1) S x4)))])) | x3]))) | x0])
\end{minipage}}
\end{center}

\noindent
Two things are worth noticing about that block. It is \emph{finite and
complete} --- there is no elision but the contentless one, and no appeal to a
viewer. And it contains no operation that inspects an encoding: the recursion
is $\mathsf{rec}$ over the row at a function type, the arithmetic is $+$, and
the branching is the numeral test extraction emits for $\lor$-elimination.

That last property is what the de-coding programme of Act~X bought, and it can
be counted rather than asserted. Taking the three theorems that have both a
coded and a structural proof, and counting occurrences of the coded operations
--- $\mathrm{ord}$, $\prec$, $\mathsf{tiRec}$, $\mathrm{hcut}$, $\hydra$,
$\mathrm{hord}$ --- in the structural extract:
...
\midrule
Goodstein on $\mathsf{ord}$ & $0$ & $2\times\mathrm{ord}^{\mathrm{o}}$, $1\times\mathsf{tiRec}^{\mathrm{o}}$\\
Kirby--Paris on $\mathsf{hyd}$ & $0$ & $7\times\mathrm{cut}^{\mathrm{H}}$, $2\times\mathrm{hord}^{\mathrm{H}}$, $1\times\mathsf{tiRec}^{\mathrm{o}}$\\
Hercules on $\mathsf{hyd}$ & $0$ & $7\times\mathrm{cutAt}^{\mathrm{H}}$, $1\times\mathsf{tiRec}^{\mathrm{o}}$\\
\bottomrule
\end{tabular}
\end{center}

\noindent
Zero is the whole claim. The encoding is not reduced in these programs; it is
absent from them, surviving only inside the alignment proofs that let every
certified fact about the coded version be inherited by the structural one.

%======================================================================
\section{Appendix D --- Catalogue of the thirteen \haomega{} programs}
\label{app:catalogue2}

Appendix~B catalogued the fragment's exports. This is the counterpart for
Part~II. Every row is one derivation; the realizer type is computed from the
statement by the modified-realizability assignment of Act~VIII, not chosen; and
the last column records what the build checks at every compilation.
...
\toprule
\textbf{Theorem} & \textbf{Realizer type} & \textbf{Extracted program, and what is checked}\\
\midrule
Fibonacci & \code{N -> (N x 1)} &
\code{fibExtracted}; values to $n=15$, $n=100$, and $n=1000$ (209 digits)\\
Fibonacci at type 2 & \code{(N -> N) -> (N x 1)} &
\code{hiProgram}; continuous, modulus $\max(f0,f(f0)){+}1$, associate in $\CtQ 2$\\
Pascal mod $2$ & \code{N -> (N -> (N x (1 x 1)))} &
\code{pasTag}/\code{pasDecide}; gasket rows $0$--$7$ against the value layer\\
Tower of Hanoi & \code{N -> (N x ((N -> N) x (1 x 1)))} &
\code{hanoiExtracted}; $n=10$, all $1023$ moves against a reference\\
gcd, full spec & \code{N -> (N -> (N x ((N x 1) x ((N x 1) x (N -> \dots)))))} &
\code{gcdFull}; agrees with \code{Nat.gcd}\\
Goodstein & \code{N -> (N x 1)} &
\code{goodsteinX}; stop times $[0,1,3,5]$, each certified terminal\\
Kirby--Paris & \code{N -> (N x 1)} &
\code{hydraX}; battle lengths $[0,1,3]$, each certified terminal\\
Sperner 1D & \code{N -> ((N -> N) -> (1 -> (1 -> (N x \dots))))} &
\code{spernerX}; a certified crossing --- the \emph{last} (Act~X)\\
Hercules, $\forall$-strategy & \code{N -> ((N -> N) -> (N x 1))} &
\code{herculesX}; agrees with \code{hydraX} at the fragment's own strategy\\
Hercules, any head & \code{N -> ((N -> N) -> ((N -> N) -> (N x 1)))} &
\code{herculesAnyX}; agrees at the leftmost head strategy\\
Goodstein, typed ordinals & \code{N -> (N x 1)} &
\code{goodsteinOX}; equal to \code{goodsteinX} at every evaluated input\\
Kirby--Paris, typed trees & \code{H -> (N x 1)} &
\code{hydraHX}; codes $0$--$7$, including $37$ where the coded extract overflows\\
Hercules, typed trees & \code{H -> ((N -> N) -> ((N -> N) -> (N x 1)))} &
\code{herculesTX}; $37$ at the same hydra, under three strategy pairs\\
\bottomrule
\end{tabular}
\end{center}

\noindent
Six of the thirteen are additionally covered by the certified translation of
Act~XII --- those that do not use $\mathrm{TI}(\epsz)$ --- and which six is
itself checked at every build.
