...
\subsection*{Fifteen extracted programs}
...
Square roots & $\forall q^{\mathbb{Q}}\forall n\,\forall K.\ \dots \to \exists k{<}K.\ c_k \neq c_{k+1}$\\
Uniform continuity & $\forall f^{\mathbb{Q}\to\mathbb{Q}}\forall j.\ \mathrm{Lip} \to \forall n\,\exists M.\ \dots$\\
\bottomrule
\end{tabular}
\end{center}
...
That modulus was originally written down by inspecting this one program.
It is now \emph{derived}. \texttt{Modulus.lean} supplies the continuity
existential with its witness --- $\mathrm{HasMod}\,F\,m$, where $m$ is a
function of the oracle --- together with an algebra for composing bounds:
constants cost nothing, post-composition costs nothing, two subcomputations
take the larger bound, and the single clause that consults the oracle says
that to know $f(Z\,f)$ you need enough of $f$ to compute the query point and
then $f$ at that point. Applied to the type-2 Fibonacci, the algebra returns
$f\,0 + 1$ where the hand-written bound was $\max(f\,0, f(f\,0)) + 1$ ---
correct, and strictly tighter, without looking at the program.

Why the bound must be a \emph{function} of the oracle rather than a number is
itself provable, and proved: for every candidate $n$ there are oracles
agreeing below $n$ that this program separates. So the existential cannot be
Skolemized to a constant, and discharging it by choice would yield a witness
that no longer computes --- which is the whole point of having one.

What is \emph{not} claimed is a metatheorem. Extraction is not proved to
yield a modulus for every derivation, and two obstructions say why that is a
research problem rather than an omission. At arrow types a single number does
not suffice: the general statement needs a bound whose \emph{type} is
computed from the finite type --- a Kleene associate --- which is exactly the
construction this act's logical relation exists to avoid. And $\mathsf{tiRec}$
admits no compositional bound at all, because the value at $x$ may consult
every $y \prec x$ and a notation can have infinitely many $\prec$-predecessors:
continuity survives, since for each fixed oracle the descending chain is
finite, but the bound stops being a formula in the subterms' bounds. That is
the same frontier Act~XII meets from the other side.
...
Transfinite recursion is covered too, and it is the interesting case. The
target gains $\mathsf{tiRec}$ with a \emph{guarded caller}: the step is
applied to the index and to a function that recurses when the order test
succeeds and returns a carried default when it does not --- the source
recursor's $\mathsf{dite}$, transcribed. (The default has to be carried
because an untyped target cannot rebuild a type-indexed $\mathsf{dflt}$; what
is emitted is the translation of the source's, proved correct by induction on
the type.) The correctness case then goes by well-founded induction on the
ordinal, the same device soundness and continuity already use. All
\emph{thirteen} programs are covered, and that is checked at every build.
...
output; and one thing more, which the transfinite case introduces.
\textsc{ghc} does not check that the emitted recursion terminates. It does
terminate --- by the $\epsz$ descent proved on the Lean side --- and the
theorem is relative to that proof. This is the ordinary shape of a
compiler-correctness statement when the target's type system is weaker than
the source's, and it is worth naming rather than eliding: the guarantee that
the emitted Goodstein program halts is a theorem in Lean, not a property
\textsc{ghc} could confirm.
...
One last observation, because it is the shape of what comes next. Sperner's
lemma in one dimension \emph{is} the discrete intermediate value theorem, and
the development's first theorem of analysis --- square roots to any precision
--- turned out to be that theorem applied to squaring, with the extracted
program a root-finding search that returns $181/128$ for $\sqrt2$ at
precision $2^{-8}$. It needed no new reasoning rules at all, because
$\mathsf{spernerD}$ quantifies over its colouring as a \emph{function
variable} and therefore never inspects the arithmetic inside it. The higher
types that Part~II was built to obtain are what made a discrete combinatorial
theorem reusable as an analytic one; the reuse was not designed for, and that
is the better kind of evidence that the types were the right thing to add.

Uniform continuity, proved next, makes the point about types sharper still.
Its statement is $\forall n\,\exists M.\,\dots$, and the
modified-realizability assignment sends that to $\Nat \to \Nat \times
\dots$ --- so the first component of the extracted realizer, as a function of
the precision, \emph{is} the modulus of continuity. Not by an encoding or an
adapter: by the type assignment Act~VIII displays. Measured, the extract
returns $n{+}1$ for doubling and $n$ for a translation, which are the moduli
one would write by hand. Whatever else constructive analysis will need from
this framework, its central object arrives for free.
