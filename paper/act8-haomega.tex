...
first-order development's proven, choice-free value layer), plus one genuinely
new construct:
\[
...
normalized, emitted as Haskell, and reasoned about --- the three-view
artifact below is just three renderings of this one certified object.

\subsection*{Nine theorems, each extracted and run}
...
Three of these the fragment could not even \emph{state}: Hercules and
Sperner quantify over functions, and the type-2 Fibonacci is where the
continuity theorem finally has content ($\mathsf{Continuous2}$ of the
extracted functional, with an explicit computable modulus
$\max(f\,0, f(f\,0))+1$). Two scope statements, verbatim from the audit:
independence from \textsc{pa} is formalized nowhere; and
$\mathrm{play}(f,h,t)$ quantifies the \emph{replication} strategy --- the
head choice stays the value layer's, and the fully general any-head theorem
is not derived.
